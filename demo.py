...
parser = argparse.ArgumentParser(
    description='Mutate sentence and mitigate violations')
...
def mutate(sentence, epsilon):
    ...
    #     candidates_with_fluency = [
    #         (fluency_scorer.score_sentence(c).item(), c) for c in candidates]
    ...
    sentences_vec = sequence.pad_sequences(
        sentences_seq, maxlen=MAX_LEN, padding="post", value=0)
    ...
    print(sentence)
    num_violations = sum([1 for i in range(
        len(testcase_scores)) if testcase_scores[i] * original_score < 0])
    print("# violations: ", num_violations)
    print("# violations (after mitigation): ",
          mitigation(predictions, epsilon))
...
def mitigation(predictions, epsilon=2):
    ...
    for i in predictions[1:]:
        score = np.sum(predictions) / (np.e**epsilon + k) + \
            np.e**epsilon * i / (np.e**epsilon + k)
